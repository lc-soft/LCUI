...
int main(int argc, char *argv[]) 
{
	...
	for(i=0; i<5; ++i) {
		...
		RadioButton_Text( rdbtn[i], "%s", text[i] );
		Widget_Show( rdbtn[i] );
	}
	/* 为单选框建立互斥关系 */
	...
	/* 为按钮的点击事件关联回调函数 */
	Widget_ClickedEvent_Connect( btn_ok, switch_style, NULL );
	/* 显示部件 */
	Widget_Show( btn_ok );
	Widget_Show( label );
...
/* 主函数，程序的入口 */
{
	...
	Widget_Event_Connect( Window_GetCloseButton(windows[0]), EVENT_CLICKED, callback );
	LCUI_Main(); /* 进入主循环 */ 
	return 0;
}
#endif

...
void test_segoe_ui_font_load(void)
{
	...
	if (id > 0) {
		...
		it_i("check font style", font->style, PD_FONT_STYLE_NORMAL);
		it_i("check font weight", font->weight, PD_FONT_WEIGHT_NORMAL);
	}
	...
	if (id > 0) {
		font = pd_font_library_get_font(id);
		it_i("check font style", font->style, PD_FONT_STYLE_ITALIC);
		it_i("check font weight", font->weight, PD_FONT_WEIGHT_NORMAL);
	}
	...
	if (id > 0) {
		font = pd_font_library_get_font(id);
		it_i("check font style", font->style, PD_FONT_STYLE_NORMAL);
		it_i("check font weight", font->weight, PD_FONT_WEIGHT_BOLD);
	}
	...
	if (id > 0) {
		font = pd_font_library_get_font(id);
		it_i("check font style", font->style, PD_FONT_STYLE_NORMAL);
		it_i("check font weight", font->weight, PD_FONT_WEIGHT_LIGHT);
	}
	...
	if (id > 0) {
		font = pd_font_library_get_font(id);
		it_i("check font style", font->style, PD_FONT_STYLE_ITALIC);
		it_i("check font weight", font->weight,
		     PD_FONT_WEIGHT_EXTRA_LIGHT);
	}
}
...
void test_arial_font_load(void)
{
	...
	if (id > 0) {
		font = pd_font_library_get_font(id);
		it_i("check font style", font->style, PD_FONT_STYLE_NORMAL);
		it_i("check font weight", font->weight, PD_FONT_WEIGHT_BLACK);
	}
...
void test_font_load(void)
{
	...
	/* 测试是否能够从字体文件中载入字体 */
	it_i("check pd_font_library_load_file success",
	...
	it_b("check pd_font_library_get_font_id success",

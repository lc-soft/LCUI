...
static void test_default_settings(void)
{
	...
	ctest_equal_int("check default frame rate cap", settings.fps_cap, 120);
...
static void test_apply_settings(void)
{
	...
	lcui_get_settings(&settings);
	ctest_equal_int("check frame rate cap", settings.fps_cap, 60);
	...
	ctest_equal_int("check frame rate cap minimum", settings.fps_cap, 1);
	...
	lcui_get_settings(&settings);
	ctest_equal_int("check frame rate cap", settings.fps_cap, LCUI_MAX_FRAMES_PER_SEC);
	lcui_quit();
	lcui_main();
}
...
void test_settings_fps_cap(void)
{
	...
	settings.fps_cap = 30;
	lcui_apply_settings(&settings);
	ptk_set_timeout(2000, check_settings_fps_cap,
			&settings.fps_cap);
	lcui_main();
...
void test_settings(void)
{
	...
	ctest_describe("test settings.fps_cap", test_settings_fps_cap);
}

...
void app_window_fullscreen(app_window_t *wnd)
{
        DWORD style = GetWindowLong(wnd->hwnd, GWL_STYLE);
        MONITORINFO mi = { sizeof(mi) };

        if (GetWindowPlacement(wnd->hwnd, &wnd->placement) &&
            GetMonitorInfo(
                MonitorFromWindow(wnd->hwnd, MONITOR_DEFAULTTOPRIMARY), &mi)) {
...
void app_window_unfullscreen(app_window_t *wnd)
{
        DWORD style = GetWindowLong(wnd->hwnd, GWL_STYLE);

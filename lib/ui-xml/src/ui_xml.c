...
/*
 * lib/ui-xml/src/ui_xml.c: -- the GUI build module, parse UI config code and
 * build UI.
...
static int ui_builder_parse_widget_node(xml_parser_t *parser, xmlNodePtr node)
{
        ...
        for (prop = node->properties; prop; prop = prop->next) {
                ...
                if (check_prop_name(prop, "id")) {
                        ...
                        prop_name = y_strdup((const char *)prop->name);
                        y_strlower(prop_name, (const char *)prop->name);

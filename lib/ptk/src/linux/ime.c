...
/* Shared by both X11 and Wayland: convert KEYPRESS key.code (Unicode codepoint)
 * to a wchar_t and commit it through the IME layer. */
static void on_key_press(ptk_event_t *e, void *arg)
{
        wchar_t text[2] = { (wchar_t)e->key.code, 0 };
...
#endif
        if (ptk_get_app_id() == PTK_APP_ID_LINUX_WAYLAND) {
                /* Wayland backend: PTK_EVENT_KEYPRESS carries the Unicode
                 * codepoint in key.code (written by xkb_state_key_get_utf32).
                 * Register directly — no full IME handler needed for basic
                 * ASCII/Unicode input without a compositor text-input protocol.
                 */
                ptk_on_event(PTK_EVENT_KEYPRESS, on_key_press, NULL);
                return 0;
        }

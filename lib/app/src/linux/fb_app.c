...
static void fb_app_init_root_window(void)
{
	...
	Graph_Init(&wnd->canvas);
	LinkedList_Init(&wnd->rects);
	fbapp.window_count = 0;
	wnd->canvas.color_type = LCUI_COLOR_TYPE_ARGB;
...
static void fb_app_init_canvas(void)
{
	...
	case 32:
		fbapp.canvas.color_type = LCUI_COLOR_TYPE_ARGB8888;
		break;
	case 24:
		fbapp.canvas.color_type = LCUI_COLOR_TYPE_RGB888;
		break;
	case 8:
...
static void fb_app_window_sync_rect16(LCUI_Graph *canvas, int x, int y)
{
	uint32_t iy, ix;
	LCUI_Rect rect;
...
static void fb_app_window_sync_rect8(LCUI_Graph *canvas, int x, int y)
{
	uint32_t ix, iy;
	LCUI_Rect rect;
	LCUI_ARGB *pixel, *pixel_row;
...
static void fb_app_window_sync_rect24(LCUI_Graph *canvas, int x, int y)
{
	Graph_Replace(&fbapp.canvas, canvas, x, y);
}

static void fb_app_window_sync_rect32(LCUI_Graph *canvas, int x, int y)
{
	Graph_Replace(&fbapp.canvas, canvas, x, y);
}

static void fb_app_window_sync_rect(app_window_t *wnd, LCUI_Rect *rect)
{
	int x, y;
	LCUI_Graph canvas;
	LCUI_Rect actual_rect;
	...
	Graph_Quote(&canvas, &wnd->canvas, &actual_rect);
...
static void fb_app_destroy(void)
{
	...
	default:
		Graph_Free(&fbapp.surface.canvas);
		break;
	}

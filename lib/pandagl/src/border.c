                              ...
                              const pd_corner_flags_t *flags)
{
        ...
        double circle_center_y =
            bound_top +
            (flags->is_bottom ? bottom_corner_height - 1.0 * radius - 0.5 : r);
        double circle_center_x =
            bound_left + (flags->is_right ? width - 1.0 * radius - 0.5 : r);
        double split_slope =
            xline->width > 0 ? 1.0 * yline->width / xline->width : 0.0;
        double split_center_x =
            bound_left +
            (flags->is_right ? width - 1.0 * yline->width : 1.0 * yline->width);
        double split_center_y =
            bound_top + (flags->is_bottom
                             ? bottom_corner_height - 1.0 * xline->width
                             : 1.0 * xline->width);
        double inner_ellipse_limit =
            flags->is_bottom ? circle_center_y + radius_y : split_center_y;
        ...
        for (y = 0; y < rect.height; ++y) {
                ...
                if (r > 0 &&
                    (flags->is_bottom ? circle_y <= 0 : circle_y >= 0)) {
                        int should_draw_inner_ellipse = !flags->is_bottom ||
                                                        !flags->is_right ||
                                                        y >= circle_center_y;
                        outer_x =
                            flags->is_right
                ...
                if (xline->width > 0) {
                        split_x = split_center_x +
                                  split_sign * ToGeoY(y, split_center_y) *
                                      split_slope;
                }
                               ...
                               const pd_corner_flags_t *flags)
{
        ...
        for (yi = 0; yi < rect.height; ++yi) {
                ...
                outer_xi =
                    flags->is_right ? (int)(center_x + x) : (int)(center_x - x);
                if (flags->is_right) {
                        outer_xi = y_max(0, outer_xi);
                        xi = outer_xi;
                        p = pd_canvas_pixel_at(dst, rect.x + outer_xi,
                                               rect.y + yi);
                } else {
                           ...
                           const pd_rect_t *box)
{
        ...
        struct {
                ...
                },
                {
                    ...
                    .height =
                        border->bottom_right_radius - border->bottom.width,
                },
        };

...
void pd_text_style_merge(pd_text_style_t *base, pd_text_style_t *target)
{
        ...
        if (pd_font_replace_style(base->font_ids, base->style, &font_ids) > 0) {
                free(base->font_ids);
        ...
        if (pd_font_replace_weight(base->font_ids, base->weight, &font_ids) >
            0) {
                free(base->font_ids);
                base->font_ids = font_ids;
        }
...
int pd_text_style_set_weight(pd_text_style_t *ts, pd_font_weight_t weight)
{
        ...
        ts->weight = weight;
        ts->has_weight = true;
        if (pd_font_replace_weight(ts->font_ids, weight, &font_ids) > 0) {
                free(ts->font_ids);
...
int pd_text_style_set_style(pd_text_style_t *ts, pd_font_style_t style)
{
        ...
        ts->style = style;
        ts->has_style = true;
        if (pd_font_replace_style(ts->font_ids, style, &font_ids) > 0) {
                free(ts->font_ids);
                ts->font_ids = font_ids;
                return 0;
        }
...
int pd_text_style_set_font(pd_text_style_t *ts, const char *const *names)
{
        ...
        ts->font_ids = NULL;
        ts->has_family = false;
        count = pd_font_query(&ts->font_ids, ts->style, ts->weight, names);
        if (count > 0) {
                ts->has_family = true;
                return 0;
        }

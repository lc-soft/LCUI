/*
 * lib/pandagl/tests/test_canvas_mix.c
 *
...
void test_pandagl_canvas_mix(void)
{
        char rgba_str[64];
        pd_color_t *pixel;
        ...
        color2str(rgba_str, pixel);
        ctest_equal_str("white_layer(0, 0)", rgba_str,
                        "rgba(255, 255, 255, 1)");

        pixel = pd_canvas_pixel_at(&white_layer, 15, 15);
        color2str(rgba_str, pixel);
        ctest_equal_str("white_layer(15, 15)", rgba_str,
                        "rgba(255, 127, 127, 1)");

        pixel = pd_canvas_pixel_at(&white_layer, 25, 25);
        color2str(rgba_str, pixel);
        ctest_equal_str("white_layer(25, 25)", rgba_str,
                        "rgba(152, 127, 229, 1)");

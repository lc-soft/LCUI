 ...
 * Liu Chao <i@lc-soft.io> All rights reserved.
 *
...
int main(void)
{
        ...
        ret = pd_font_load_file("C:/Windows/fonts/simsun.ttc");
        while (ret == 0) {
                fid = pd_font_get_id("SimSun", 0, 0);
                if (fid < 0) {
                        break;
                }
                ret = pd_font_render_glyph(&bmp, L'字', fid, 48);
                if (ret != 0) {
                        break;
                }
                pd_canvas_mix_glyph_bitmap(&img, pos, &bmp, color);
                pd_write_png_file("test_char_render.png", &img);
                pd_glyph_bitmap_destroy(&bmp);

...
static char *lcui_join_path(const char *left, const char *right)
{
	...
	strcpy(path, left);
	if (left_len > 0 && path[left_len - 1] != '/' && path[left_len - 1] != '\\') {
		path[left_len] = '/';
		path[left_len + 1] = 0;
	}
	strcat(path, right);
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		*equal++ = 0;
		...
		if (strcasecmp(section, "meta") == 0 &&
			...
			has_version = true;
			if (!lcui_store_parse_int(equal, &value) || value != 1) {
				*dirty = true;
			}
			continue;
		}
...
static bool lcui_store_save(const char *path, const lcui_store_rendering_t *settings)
{
	size_t path_len;
	char *tmp_path;
	FILE *fp;
...
static bool lcui_settings_validate_rendering(const lcui_rendering_settings_t *settings)
{
	if (!settings) {
		return false;
	}
...
static void lcui_settings_apply_rendering(void)
{
	...
	ui_server_set_paint_flashing_enabled(lcui_settings.rendering.paint_flashing);
...
bool lcui_settings_load(void)
{
	...
	if (!lcui_store_load(path, &lcui_settings.rendering, &lcui_settings.dirty,
			     &exists)) {
		free(path);
		return false;
	}
	if (!exists || !lcui_settings_repair_rendering(&lcui_settings.rendering)) {
...
{
	if (!lcui_settings.loaded || !in || !lcui_settings_validate_rendering(in)) {
		return false;
	}
...
void lcui_get_settings(lcui_settings_t *settings)
{
	...
	settings->parallel_rendering_threads = lcui_settings.rendering.parallel_threads;
...
void lcui_apply_settings(lcui_settings_t *settings)
{
	...
	if (!lcui_settings_validate_rendering(&rendering)) {
		lcui_store_rendering_t repaired = {
			rendering.fps_cap,

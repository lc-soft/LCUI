...
void lcui_set_ui_display_mode(lcui_display_mode_t mode)
{
        ...
        case LCUI_DISPLAY_MODE_FULLSCREEN:
                if (lcui_ui.mode != mode) {
                        ui_widget_resize(root, lcui_ui.width_backup,
                                         lcui_ui.height_backup);
                }
        case LCUI_DISPLAY_MODE_WINDOWED:
                ui_server_disconnect(root, NULL);
                break;
        ...
        switch (mode) {
        case LCUI_DISPLAY_MODE_FULLSCREEN:
		logger_debug("[lcui-ui] fullscreen mode\n");
                if (lcui_ui.mode != mode) {
                        lcui_ui.width_backup = root->border_box.width;
                        lcui_ui.height_backup = root->border_box.height;
                }
                wnd = app_window_create(NULL, 0, 0, 0, 0, NULL);
                ui_server_connect(root, wnd);
                list_append(&lcui_ui.windows, wnd);
                app_window_fullscreen(wnd);
                break;
        case LCUI_DISPLAY_MODE_SEAMLESS:
                options.child_list = TRUE;
                lcui_ui.observer =
                    ui_mutation_observer_create(lcui_on_ui_mutation, NULL);
                ui_mutation_observer_observe(lcui_ui.observer, root, options);
                for (list_each(node, &root->children)) {
                        wnd = app_window_create(NULL, 0, 0, 0, 0, NULL);
        ...
        case LCUI_DISPLAY_MODE_WINDOWED:
        default:
		logger_debug("[lcui-ui] windowed mode\n");
                wnd = app_window_create(NULL, 0, 0, 0, 0, NULL);
		logger_debug("[lcui-ui] created window: %p\n", wnd);
                ui_server_connect(root, wnd);
                break;
        }
        lcui_ui.mode = mode;
...
void lcui_init_ui(void)
{
        ...
        lcui_ui.image_loading_timer =
            lcui_set_interval(100, lcui_ui_image_loading_timer, NULL);
}

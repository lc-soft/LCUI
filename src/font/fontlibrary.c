...
/** 初始化字体位图 */
void FontBMP_Init( LCUI_FontBitmap *bitmap )
{
	bitmap->rows = 0;
	bitmap->width = 0;
	bitmap->top = 0;
...
/** 释放字体位图占用的资源 */
void FontBMP_Free( LCUI_FontBitmap *bitmap )
{
...
/** 创建字体位图 */
int FontBMP_Create( LCUI_FontBitmap *bitmap, int width, int rows )
{
	size_t size;
	if(width < 0 || rows < 0) {
		FontBMP_Free(bitmap);
...
/** 在屏幕打印以0和1表示字体位图 */
int FontBMP_Print( LCUI_FontBitmap *fontbmp )
{
	int x,y,m;

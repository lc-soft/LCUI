...
scan_style_ending_tag( const wchar_t *wstr, char *name_buff )
{
	int i, j, len;
...
/** 在字符串中获取指定样式标签中的数据 */
static const wchar_t* scan_style_tag_by_name( const wchar_t *str, 
				const char *in_tag_name, char *out_tag_data )
{
	int i, j, len, tag_len; 

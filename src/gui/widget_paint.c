...
static int Widget_DrawBackground( LCUI_Widget widget, LCUI_PaintContext paint )
{
	Graph_FillImageWithColorEx( &paint->canvas, &widget->style.background.image,
		LAYOUT_CENTER, widget->style.background.color, paint->rect
	);
	return 0;
}

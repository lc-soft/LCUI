...
auto_resize_msgbox(LCUI_Widget *textlabel, LCUI_WidgetEvent *event )
{
	...
	btn_type = msgbox->self_id ^ WIDGET_ID_MESSAGEBOX;
	switch( btn_type ) {
	case MB_BTN_OK:
		min_width = 150;
		break;
	...
	if( (msgbox->self_id & MSGBOX_NONE_ICON) == MSGBOX_NONE_ICON ) {
		msgbox_size.h = msgbox_size.w = area_height = 10;
	} else {
	...
	_DEBUG_MSG("widget: %p, btn_type: %d, old size: (%d,%d), new size: (%d,%d)\n",
		textlabel, btn_type, 
...
static void
msgbox_add_BTN_OKCANCEL( LCUI_Widget *msgbox )
{
...
static void
msgbox_add_BTN_RETRYCANCEL( LCUI_Widget *msgbox )
{
	
}

static void
msgbox_add_BTN_YESNO( LCUI_Widget *msgbox )
{
	
}

static void
msgbox_add_BTN_YESNOCANCEL( LCUI_Widget *msgbox )
{
			...
			const char *title, MB_BTN_TYPE button )
{
	...
	if( icon_type == MB_ICON_NONE ) {
		...
		Widget_SetAlign( textbox, ALIGN_MIDDLE_LEFT, Pos(0,0) );
	} else {
		icon_flag = 0;

...
struct _LCUI_TextBox
{
	...
	LCUI_Queue tag_buff;		/* 用于保存样式标签中表达的属性数据 */
	LCUI_Queue style_data;		/* 临时保存记录标签 */
...
static void Destroy_FontData(LCUI_FontData *data)
{
	String_Free( &data->family );
}
...
/* 将字体样式数据加入队列 */
{
	...
	Queue_Add( &textbox->tag_buff, data );
}
...
/* 获取当前的字体样式数据 */
{
	...
	total = Queue_Get_Total( &textbox->tag_buff );
	if(total > 0) {
		/* 从样式数据队列中获取字体样式数据 */
		for(equal=0,i=total-1; i>=0; --i) {
			p = Queue_Get( &textbox->tag_buff, i );
			switch( p->tag ) {
			    case TAG_ID_COLOR: 
				if( flags[0] == 0 ) {
...
/* 将指定标签的样式数据从队列中删除，只删除队列尾部第一个匹配的标签 */
{
	...
	if(total > 0) {
		for(i=total-1; i>=0; --i) {
			p = Queue_Get( &textbox->tag_buff, i );
			if( p->tag == tag ) {
				Queue_Delete( &textbox->tag_buff, i );
			}
		}
	}
}
...
static wchar_t *covernt_tag_to_style_data (wchar_t *str, tag_style_data *out_data)
...
/* 初始化文本框相关数据 */
{
	...
	Queue_Init( &textbox->tag_buff, sizeof(tag_style_data), 
			destroy_tag_style_data );
	Queue_Init( &textbox->style_data, sizeof(LCUI_FontData), 
			Destroy_FontData );
			...
			if( p_data->data != NULL 
			 && p_data->data->need_fore_color ) {
				color = p_data->data->fore_color;
				 DEBUG_MSG("custom RGB: %d,%d,%d\n", color.red, color.green, color.blue);
			} else {
				color = widget->fore_color;
			}
			FontBMP_Mix( &widget->graph, pos.x, pos.y,
					&p_data->bitmap, color, 
	...
	for(p=buff, finish=buff+total; p<finish; ++p) { 
		...
		if(n_ignore > 0) { 
			...
			while(n_ignore--) { 
				...
				DEBUG_MSG("current pos: %d, char %c\n", textbox->current_src_pos, char_data.char_code);
		...
		DEBUG_MSG("1, at queue, pos: %d, char: %c, data: %p\n", 
		...
		DEBUG_MSG("2, at queue, pos: %d, char: %c, data: %p\n", 
